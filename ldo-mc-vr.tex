...
This work aims to characterize the performance of Monte Carlo biasing
...
in angle incorporate angular information into the resultant scalar flux
solutions and corresponding Monte Carlo biasing parameters. The results for
several test problems are compared to results where the biasing parameters
...
code \cite{denovo} in the Exnihilo framework, with Monte Carlo biasing
...
\cite{cadis} in the MCNP code. The results of using the CADIS method are
...
A key result of the CADIS method is that the statistical weights of the source
particles are the weight windows centers. In other words, the source-biasing
parameters and the weight window target values are consistent. This
circumvents the potential of particles being immediately split or rouletted
upon birth and avoids the resultant degradation in computational efficiency.
We refer the reader to Reference \cite{cadis} for a complete discussion of
results and analysis of the initial implementation of the CADIS method. The
CADIS method is very effective for automatically improving calculations for
localized responses, but is not designed for distributed responses. \fwc,
discussed in the next section, was developed to improve distributed response
calculations.
...
Carlo calculations of global quantities; the details of this method can be
found in Reference \cite{fwcadis}. For this global variance reduction method,
...
The discrete ordinates, or \sn, approximation is the most commonly-used
discretization for angle to deterministically solve the steady-state Boltzmann
transport equation:
%
\begin{multline}
...
approximation gives one equation for each angle, index $n$:
%
\begin{multline}
...
\end{equation}
%
where $w_n$ are the integration weights and $N$ is the number of ordinates. We
refer the reader to Reference \cite{denovo} and the references therein for a
complete discussion of the derivation of the \sn\ equations as well as
solution methods for the equations.
...
travel can be important, Peplow introduced directional CADIS in 2012
...
standard CADIS are a neutron porosity tool problem and a gamma-ray litho-
density tool problem with the detector far away from the source. It should
...
than an analog Monte Carlo calculation. For full details on the test results,
we refer the reader to Reference \cite{peplow}. In the conclusions of the
...
three categories of processes that affect particle flux anisotropy: strongly-
...
Similar to the conclusions drawn by Peplow, Munk found that it was difficult
...
In this Section, we introduce and describe the test case scenarios examined and the various parameters chosen for the calculations in this work.
% what are these parameters compared to the MC and deterministic parameters? What else is there?
...
% Add a description of why this problem is interesting for this work
...
the remaining available tasks \cite{mcnp}. For the purpose of parallel
...
was modeled after calculations in Reference \cite{sw-dlvn}. All Monte Carlo
tally results, $\xbar$, are reported with the one standard deviation
confidence interval, $\xbar(1\pm R)$, where the relative error, $R$, is
defined as $S_{\xbar}/\xbar$ \cite{mcnp}. % define S_{\xbar}

In the \fwc\ calculations for the DLVN and simplified portal monitor
scenarios, we examine Monte Carlo results corresponding to biasing parameters
from a chosen representative subset of the quadrature sets. For these
comparisons, quadrature sets of similar angular mesh refinement were chosen
such that the quadrature sets have approximately the  same total number of
angles, with the exception of the Galerkin quadrature set. The QR quadrature
set is of order 4 and has 128 angles, the LDFE set is order 1 with 128 angles,
and the LDO set is of order 11 with 144 angles.  The Galerkin quadrature set
chosen as the representative example here is of order 4 and has 24 angles.
This set was chosen because its corresponding \pn\ order is 5 and so the
scattering data used matches that of the other quadrature types.
...
Monte Carlo calculations with biasing parameters from the Galerkin quadrature
...
the order of $10^{-12}$, while the unbiased tally calculation is on the order
...
of the number of discrete angles used. This indicates that any improvement
gained by more accurate angular information is offset by the increased
computational cost. The Figures of Merit vary somewhat more greatly.
Specifically, the LDO biasing parameters appear to gather around FOM values of
0.005 even as the number of discrete angles used is increased. Thus, for the
steel plate in water problem solved with the CADIS method, one could use a
relatively low-order (\textit{i.e.}, order 8) LDO quadrature set to generate
Monte Carlo biasing parameters that result in a Figure of Merit comparable to
or better than those produced by finer angular meshes.
...
The Monte Carlo calculation with biasing parameters from the Galerkin
...
tally results all fall far below that of the unbiased calculation. Again, the
angular mesh refinement has little impact on the tally results. If using an
LDO quadrature set to generate biasing parameters with the \fwc\ method in a
similar scenario, a low-order LDO angular mesh could be used to good effect.
...
number of angles used in generating the biasing parameters. The average FOM for
the unbiased calculation is shown as a horizontal black line.
...
refinement; the coarsest and finest angular meshes produce Monte Carlo biasing
...
above, the LDO quadrature set of order 8 is the best choice with respect to
...
Results from the Monte Carlo calculation with VR are also included for
...
tallies do not match the experimentally-measured flux values from using CADIS with any of the
representative quadrature sets or the no-VR calculation, so we only compare
...
% universally, we need a different word than biased. You're not biasing the calculations. You're using variance redution. They're not biased results. Can you go through and update? I'm getting tired enough that I don't have a good idea. I'm sorry I didn't notice that before. 
