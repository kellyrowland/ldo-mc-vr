...
Hybrid radiation transport methods that use
deterministic solutions to speed up stochastic calculations are rarely effective for shielding problems
with highly anisotropic particle movement and particle streaming pathways.
In this work, we investigate using a different deterministic formulation, the Lagrange Discrete Ordinates (LDO), for variance reduction parameter generation in hybrid methods. 
The LDO equations retain the formal structure of
the traditional discrete ordinates approximation of the Boltzmann transport
...
effects at increased angular resolutions. Here, we assess the LDO equations' scalar flux solutions as input in the
CADIS and \fwc\ methods for Monte Carlo variance reduction parameter
generation. Monte Carlo calculations and Figures of Merit using biasing
parameters from the LDO equations are compared against those using biasing
parameters from standard quadrature set types for a small variety of test case
scenarios in both neutron and photon transport. The LDO equations' variance
reduction parameters see their best performance in the \fwc\ method,
especially for photon transport.
...
Monte Carlo methods to achieve more accurate results more quickly. The CADIS (consistent adjoint
...
generation for shielding. However, these methods do not entirely mitigate the negative aspects of the combined simulation types.

One particular area of interest where even hybrid methods struggle to yield accurate results quickly is in
...
equations. 
We employ the LDO equations' solutions in the standard
...
given regarding the CADIS and \fwc\ methods as well as about the LDO equations. Test case scenarios are then described with relevant calculation
...
% you might want to indicate in this first paragraph that CADIS is for local response values. 
...
shown that
% you might want to put a ref for the adjoint identity so someone can easily look it up if they don't know what that means. 
...
% I think the above is well explained, but might be more than is actually needed. That is, a reference to CADIS papers might be sufficient and you could just show the punchline of what will be needed / used. (see, e.g. the paper I wrote with Steve Wilson.). Mostly I'm just not sure the reader really needs all of this background information to understand the work. Same with FW-CADIS. Does that make sense? 
% Converseley, it's not that long and it probably makes it easier to understand, so maybe it's fine.  
...
\label{eq:cadis_tb}
\end{equation}
% why don't we just show how to calculate the centers?
...
If the average particle weight is set to be proportional to the physical particle
...
\noindent By next defining the adjoint source as the bracketed term in the
above equation,
...
% you might want to suggest a comparison reference for the Sn equations. If you're not familiar with them, none of this will make any sense. 
...
% It feels weird that you describe so much less of this than the CADIS stuff. 
% The amount of information seems a bit out of sync. 
% What I would add here is some description about LDO. No one knows about these and they don't know what they are, how they perform, etc. I'd give some quick descriptions and we should come up with a way to reference your other paper. Depending on the submission timing, we can start by putting that other paper on arxiv after it's submitted and referencing it that way.

% It would be useful to motivate this work a little bit more. I'd add a section here that is past work as a small subsection. Describe some examples of where cadis and fw-cadis didn't work well and what some others have done to try to resovle the problem. That will put this work into context. Otherwise, it's hard to understand what problem you're really solving and if anyone else has looked at this at all. 
...
% per steven, add why each problem was chosen. 
...
% are there other ways to do it such that this was a choice in exnihilo?
