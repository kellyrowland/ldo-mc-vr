...
Discrete Ordinates (LDO), for variance reduction (VR) parameter generation in
...
of Merit using VR parameters from the LDO equations are compared against
those using VR parameters from standard quadrature set types for a small
...
technique is applied. Weight window lower bounds $w_{\ell}$ must be
calculated such that the statistical particle weights defined fall at the
center of the weight window intervals. The width of a given interval is
denoted by $c_u = w_u/w_{\ell}$, the ratio of upper and lower weight window
values. The weight window lower bounds are then given by
%
\begin{equation}
...
particles are at the weight windows' centers. In other words, the source-biasing
...
solutions exhibit slow Monte Carlo error reduction; we aim to discern if
angular mesh refinement in generating VR parameters has any impact on Monte
Carlo error reduction as well as how the LDO equations perform in this regard
when compared to standard quadrature types. A diagram of the problem geometry
is shown in Figure \ref{steelxz} and a list of material properties used in the
problem is given in Table \ref{steel-mat}. In Figure \ref{steelxz}, the orange
region contains the source material, the black region is composed of steel,
the blue regions indicate water, and the white region is composed of air.
...
defined as $S_{\xbar}/\xbar$ and $S_{\xbar}$ is the standard deviation of the
tally result estimate \cite{mcnp}.
...
Monte Carlo calculations with variance parameters from the Galerkin quadrature
...
that the resultant Monte Carlo flux tally results all fall far below that of
...
and Figure of Merit values resultant from calculations using VR parameters.
...
\\
No VR  & 0.149122 & 0.27845 & 14.555 & 2.52563 & 0.453763 & 0.76362
\end{tabular}
\end{table}
...
angles. At all detector locations except for the \nth{2} in the array, the no-
VR calculation and the calculations with VR parameters resultant from the
representative quadrature sets all match within statistical uncertainty. The
\nth{2} detector sees all representative quadrature set VR flux tallies
...
\\
No VR & 4.40963 & 3.15204 & 2.096 & 2.8313
\end{tabular}
\end{table}

To conclude, using LDO quadrature sets to generate Monte Carlo variance reduction
...
for any of the neutron transport scenarios, a low-order (3 - 8) quadrature set
